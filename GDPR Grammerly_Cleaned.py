...
#To remain unchanged
def obtainTerms():
    defs = pd.read_excel("Definitions.xlsx")
    ...
    other = (pd.read_excel("TermsList_Preliminary_v.1.xlsx").
...
#To remain unchanged
def obtainLegalDoc():
    document = (pd.read_excel("GDPR ChapterArticleSections.xlsx").reset_index().
...
termColorDict = dict(pd.read_excel("ColorDictionary.xlsx")["Corresponding Color"]) #Just for cosmetic purposes
